...
class MiniCPMOVisionModel(SiglipPreTrainedModel):
    def __init__(self, config: MiniCPMOVisionConfig):
        super().__init__(config)
        self.config = config
        embed_dim = config.hidden_size
...
class MiniCPMOMultiModalResampler(nn.Module):
    ...
    def __init__(self, config: MiniCPMOResamplerConfig):
        ...
        self.query = nn.Parameter(torch.zeros(config.num_query_tokens, config.hidden_size))
    ...
    def forward(self, image_features, tgt_sizes=None):
